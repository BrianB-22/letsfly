...
- **Real turbulence callout, wired to an actual dataref** — the "Turbulence" callout was removed (2026-07-14) after live-flight testing showed it was firing once on IMC entry (`clouds = turbulence in practice`, a proxy that was never accurate) and, worse, G-force data over a genuinely rough 5-minute stretch of a real flight showed *zero* variance (Gn sat in a flat 0.94-1.05 band despite the pilot feeling turbulence with 51% rain) — confirming G-force isn't a usable signal either at 1Hz. The app has never read an actual turbulence dataref. Candidate untested: `sim/weather/region/turbulence` (array of 3, same family as the confirmed-working `cloud_base_msl_m`/`cloud_coverage_percent` in `memory/xp12-web-api-spec.md`). Needs live X-Plane session + the `CheckRide-XP12-API-Scan` tool to verify the dataref resolves and returns sane values before wiring it back into `FlightMonitor.cs`.
